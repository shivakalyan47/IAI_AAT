...
def _is_low_quality(response: str) -> bool:
    ...
    - Are extremely short (< 10 chars).
    - Contain explicit AI self-identification phrases that break immersion.
    """
    # Lowered threshold to 10 to allow valid short character dialogue.
    if not response or len(response.strip()) < 10:
        return True

    # Removed generic "I cannot" or "I am unable" to allow characters to 
    # express limitations naturally. We now focus only on AI-specific identity.
    ai_phrases = [
        "i am an ai",
        "i'm an ai",
        "as an ai",
